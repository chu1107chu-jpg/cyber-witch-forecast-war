...
# Матрица эффективности типов (атакующий → защитник → мультипликатор)
TYPE_CHART: dict[str, dict[str, float]] = {
    "Берсерк":  {"Саппорт": 1.25, "Танк": 0.80, "Контроль": 0.85},
    "Танк":     {"Берсерк": 1.20, "Ассасин": 0.75, "Бруйзер": 1.10},
    "Ассасин":  {"Танк": 1.30, "Бруйзер": 0.80, "Контроль": 1.10},
    "Контроль": {"Ассасин": 1.20, "Берсерк": 0.85, "Бруйзер": 1.10},
    "Бруйзер":  {"Ассасин": 1.20, "Танк": 0.85, "Саппорт": 0.90},
    "Саппорт":  {"Контроль": 1.25, "Берсерк": 0.80, "Танк": 1.10},
}
...
def _type_multiplier(attacker_type: str, defender_type: str) -> float:
...
def _compute_power_score(fighter: dict, opponent: dict) -> float:
    ...
    type_mult = _type_multiplier(fighter["type"], opponent["type"])
...
def _calc_damage(attacker: dict, defender: dict, move: dict,
                 rng: random.Random) -> tuple[int, bool, str]:
    """Считает урон одного удара. Возвращает (урон, крит?, эффект)."""
    # Проверка попадания
    ...
    # Тип-мультипликатор
    raw *= _type_multiplier(attacker["type"], defender["type"])
...
def simulate_fight(
    ...
    for rnd in range(1, ROUNDS + 1):
        ...
        dmg_b, crit_a, eff_a_raw = _calc_damage(fa, fb, move_a, rng)
        dmg_a, crit_b, eff_b_raw = _calc_damage(fb, fa, move_b, rng)
