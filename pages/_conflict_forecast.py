...
LEADER_PROFILES = {
    ...
    # ✝ Хаменеи умер (2026). Вакансия верховного лидера — КСИР де-факто управляет страной.
    "🇮🇷 Иран: вакансия власти": {
        "side": "B",
        "age": 57,                   # Мостафа Хаменеи (сын) — вероятный преемник, ~57 лет
        "hawkishness": 0.88,         # КСИР ужесточает курс, чтобы показать силу
        "domestic_approval": 0.12,  # похороны + протесты + экономика → минимум
        "election_months": 0,        # Совет экспертов выбирает, не народ
        "rationality": 0.35,         # борьба фракций КСИР / реформаторов = хаос
        "political_survival": 0.70, # преемник не закреплён, власть фрагментирована
        "legacy_seeking": 0.60,
        "why": "✝ Хаменеи умер (2026). Совет экспертов ищет преемника. "
               "КСИР (Салами) де-факто контролирует ядерную программу и РВСМ. "
               "Борьба фракций → рациональность минимальна. "
               "Исторически: вакансия лидера = +25-40% к вероятности эскалации "
               "(Иран 1989 не дал войны, но это было исключение — Хомейни победил Ирак).",
    },
}
...
ATROCITY_REGISTRY = {
    ...
    "🇮🇷 Иран": {
        "events": [
            ...
            {"year": 2026, "event": "✝ Хаменеи умер. КСИР перехватывает операционный контроль", "severity": 0.72},
        ],
        "score": 0.74,  # повышено: вакансия + КСИР
    },
}
